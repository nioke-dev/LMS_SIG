<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-zinc-100 shadow-sm">
        ...
        <a href="{{ route('sme.blueprint.index') }}" wire:navigate class="px-6 py-3 bg-zinc-100 text-zinc-600 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-zinc-200 transition-all flex items-center gap-2 flex-shrink-0 w-max">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="space-y-8">
            <div class="bg-white p-8 rounded-[2.5rem] border border-zinc-100 shadow-sm space-y-6 sticky top-8">
                ...
                <form wire:submit="submitMaterial" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="file" wire:model="materialFile" id="material-file-upload" class="hidden">
                        <label for="material-file-upload" class="p-6 bg-zinc-50 border-2 border-dashed border-zinc-300 rounded-2xl flex flex-col items-center justify-center gap-3 text-center group hover:bg-primary/5 hover:border-primary transition-all cursor-pointer block">
                            ...
                            <div>
                                @if(!$fileName)
                                    <p class="text-xs font-bold text-zinc-800">Upload File Materi Di Sini</p>
                                @else
                                    <p class="text-xs font-black text-primary truncate max-w-[220px] bg-white px-3 py-1 rounded-full border border-primary/20 shadow-sm inline-block mt-1">{{ $fileName }}</p>
                                @endif
                            </div>
                            <p class="text-[10px] text-zinc-400 font-medium">Format: PDF, DOCX, PPTX, MP4 (Maks. 50MB)</p>
                        </label>
                        <div wire:loading wire:target="materialFile" class="text-[10px] text-primary font-bold animate-pulse mt-2 text-center">Mengunggah file ke server...</div>
                        @error('materialFile') <span class="text-red-500 text-[10px] font-bold mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea wire:model="materialNotes" rows="6" placeholder="Tuliskan deskripsi lengkap, poin-poin bahasan modul, atau pesan khusus mengenai file yang diunggah..." class="w-full px-4 py-3 text-xs bg-zinc-50 border border-zinc-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all font-medium text-zinc-800"></textarea>
                        @error('materialNotes') <span class="text-red-500 text-[10px] font-bold mt-1 block">{{ $message }}</span> @enderror
                    ...
                    {{-- Submit Button --}}
                    <button type="submit" wire:loading.attr="disabled" class="w-full py-4 bg-primary text-white text-xs font-black uppercase tracking-widest rounded-2xl hover:bg-primary-container shadow-lg shadow-primary/20 hover:shadow-primary/40 transition-all active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                        <span wire:loading.remove wire:target="submitMaterial" class="material-symbols-outlined text-lg">send</span>
                        <span wire:loading wire:target="submitMaterial" class="material-symbols-outlined text-lg animate-spin">refresh</span>
                        <span wire:loading.remove wire:target="submitMaterial">Upload & Kirim Materi</span>
                        <span wire:loading wire:target="submitMaterial">Mengirim Materi...</span>
                    </button>
                </form>
            </div>
        </div>
