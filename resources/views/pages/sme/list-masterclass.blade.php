...
@section('title', 'Masterclass Curriculum Builder')

@section('page-title', 'Masterclass Builder')
...
<div class="space-y-8" x-data="{
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-zinc-100 shadow-sm">
        <div class="flex items-center gap-6">
            <div class="w-16 h-16 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center shadow-inner flex-shrink-0 border border-emerald-100">
                <span class="material-symbols-outlined text-3xl">dashboard_customize</span>
            </div>
            <div>
                <h1 class="text-2xl font-black text-on-surface tracking-tight uppercase">Masterclass Curriculum Builder</h1>
                <p class="text-xs font-bold text-zinc-500 mt-1">Rancang struktur bab, unggah video pembelajaran riil, lampirkan dokumen pendukung, dan susun kuis evaluasi untuk materi yang telah disetujui.</p>
            </div>
        </div>
            ...
            <tbody class="divide-y divide-zinc-50">
                ...
                <tr x-show="filterBlueprint('{{ addslashes($bp->title . ' ' . $bp->id) }}', '{{ addslashes($bp->category) }}')" x-transition class="hover:bg-zinc-50/30 transition-colors group cursor-default">
                    <td class="px-8 py-6 w-5/12">
                        <div class="flex items-start gap-4">
                            <div class="w-12 h-12 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-600 group-hover:bg-primary/10 group-hover:text-primary transition-colors flex-shrink-0 shadow-sm border border-emerald-100 group-hover:border-primary/20">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-zinc-400 font-bold uppercase tracking-widest mt-1">Kode: <span class="text-zinc-600">{{ $bp->id }}</span> • Persetujuan Materi Selesai</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-8 py-6">
                        <div class="flex flex-col gap-1">
                            ...
                            <span class="text-[10px] font-black uppercase tracking-widest px-2.5 py-0.5 bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-full w-max">
                    ...
                    <td class="px-8 py-6 whitespace-nowrap">
                        <div class="flex items-center gap-2 text-xs font-bold text-zinc-600">
                            ...
                            {{ $bp->deadline ? \Carbon\Carbon::parse($bp->deadline)->translatedFormat('d F Y') : '19 Mei 2026' }}
                        </div>
                    </td>
                    <td class="px-8 py-6 text-right whitespace-nowrap">
                        <a href="{{ route('sme.masterclass.curriculum', $bp->id) }}" class="inline-block px-6 py-3 bg-emerald-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-emerald-700 shadow-lg shadow-emerald-600/20 hover:shadow-emerald-600/30 transition-all active:scale-95 flex items-center gap-1.5 w-max ml-auto">
                            <span class="material-symbols-outlined text-sm">dashboard_customize</span>
                            Buka Masterclass Builder
                        </a>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="4" class="px-8 py-12 text-center text-zinc-400 text-xs italic bg-zinc-50/50">
                        Belum ada blueprint yang siap untuk tahap produksi Masterclass saat ini.
                    </td>
                </tr>
                @endforelse
            </tbody>
